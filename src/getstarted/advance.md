...
目前（截止到文档更新），SRA的最新版本最多支持 5 个账号的托管。

![0.8.0 版本中的"多账号(配置方案)"](/img/use/ma.png)

:::info 在v1.0.3中，如果游戏内已经登录账号，SRA会自动退出当前账号。
:::
...
现在，您可以回到SRA的控制台，点击 `开始` 即可开始您的多账号托管之旅。
:::
...
6. MirrorChyanCDK：如果您购买了mirror酱的CDK，将它填写到此处，更新时将使用mirror酱提供的下载加速服务。**仅能在0.8.0+版本使用**。
7. 文件完整性检查：点击此按钮后将立即进行一次文件完整性检查（依赖于SRA更新器），SRA可能会被退出。
8. 热键设置：此处修改全局热键，您可以在任何地方按下快捷键来执行这些操作。**仅能在0.8.1+版本使用**。
9. 通知：
...
10. 关闭窗口时直接退出：在0.8.1中，SRA增加了一个系统托盘，与此同时，当您点击关闭按钮时仅会关闭窗口，而不会结束SRA的进程。启用此项后点击关闭按钮时会同时停止SRA的进程。
...
3. 输入`SRA.exe`（cmd）或`.\SRA.exe`（powershell），此时SRA将以调试模式启动，您可以在命令行程序看到包括 `DEBUG` 级别和 `TRACE` 级别的所有信息。
...
::: warning 参数操作在v1.0.0-v1.0.3中不可用。开发者正在尽力适配。
:::
...
如果您使用SRA命令行只是为了执行任务，或者您只想直接执行任务不使用SRA命令行，可以在窗口中输入`SRA --run [config] [...]`直接开始执行任务。