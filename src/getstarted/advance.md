...
- **Webhook 地址**：接收通知的 HTTP 端点，SRA 会向该地址发送 POST 请求，请求体为 JSON 格式，包含事件、结果、时间和消息字段。
...
[OneBot](https://onebot.dev) 是统一聊天机器人接口标准，常用于 QQ 机器人（如 NapCat、LLOneBot 等）。
...
:::tip
建议本地部署机器人，Endpoint 填写 `http://127.0.0.1:端口号` 即可，无需公网。
:::
